    ...
    get_digital_samples(get_digital_samples) 
{
    ...
    this->lp_numerator[0] = 0.00844269f;
    this->lp_numerator[1] = 0.01688539f;
    this->lp_numerator[2] = 0.00844269f;
    
    this->lp_denominator[0] = 1.0f;
    this->lp_denominator[1] = -1.72377617f;
    this->lp_denominator[2] = 0.75754694f;

    this->rl_cos_space = pow(BANDPASS_SMOOTHING, SAMPLES_PER_SYMBOL) * cos(omega_space * SAMPLES_PER_SYMBOL * SAMPLING_PERIOD); 
    this->rl_sin_space = pow(BANDPASS_SMOOTHING, SAMPLES_PER_SYMBOL) * sin(omega_space * SAMPLES_PER_SYMBOL * SAMPLING_PERIOD); 
    ...
    this->rl_cos_mark = pow(BANDPASS_SMOOTHING, SAMPLES_PER_SYMBOL) * cos(omega_mark * SAMPLES_PER_SYMBOL * SAMPLING_PERIOD); 
    this->rl_sin_mark = pow(BANDPASS_SMOOTHING, SAMPLES_PER_SYMBOL) * sin(omega_mark * SAMPLES_PER_SYMBOL * SAMPLING_PERIOD); 
